...
'use strict';
{
	...
	if (useMZ){
		...
		if (hasPluginCommonBase){
			...
			PluginManagerEx.registerCommand(document.currentScript, "setSpinWait", function (args) {
				const character = this.character(args.id);
				if (character) {
					character.setSpinWait(args.frames === "auto" ? 0 : args.frames);
				}
			});
	...
	Game_CharacterBase.prototype.updateSpinDirection = function(spinWait) {
		...
		const turn90 = this.canReverseSpin() ? "turnLeft90" : "turnRight90";
		while(times > 0) {
			this[turn90]();
			times--;
		}
	...
	Game_Followers.prototype.jumpAll = function() {
		if ($gamePlayer.isJumping()) {
			for (const follower of this._data) {
				follower.spin($gamePlayer.canSpin());
				follower.alwaysSpin($gamePlayer.canAlwaysSpin());
			}
		}
